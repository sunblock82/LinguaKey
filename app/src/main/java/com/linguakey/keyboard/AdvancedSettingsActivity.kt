...
class AdvancedSettingsActivity : AppCompatActivity() {
    ...
    private fun translation() {
        ...
        choice("제공자",listOf("기기 내 기본 번역" to "offline","OpenAI GPT" to "openai","Google Gemini" to "gemini"),prefs.translationProvider) { prefs.translationProvider=it; render() }
        ...
        if (prefs.translationProvider != "offline") {
            ...
            button("이 제공자 API 키 삭제") { keys.save(provider,""); toast("삭제했습니다") }
        ...
        heading("연결 테스트")
        val sample=EditText(this).apply { setText("오케이 누나도 얼른 자"); minLines=2 }
        root.addView(sample)
    ...
    private fun field(label:String,value:String,multiline:Boolean=false,save:(String)->Unit) {
        note(label)
        val edit=EditText(this).apply { setText(value); if(multiline) { minLines=2; inputType=InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_FLAG_MULTI_LINE } else setSingleLine() }
